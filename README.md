...
- [1️⃣ '근처 일거리 찾기 API' 고도화 과정과 결과](#1️⃣-근처-일거리-찾기-api-고도화-과정과-결과)
  - [(0) 최초 구현](#0-최초-구현)
  - [(1) 1차 고도화: 쿼리 전략 개선](#1-1차-고도화-쿼리-전략-개선)
  - [(2) 2차 고도화: 쿼리 튜닝](#2-2차-고도화-쿼리-튜닝)
  - [(3) 3차 고도화: mysql 공간 객체 활용](#3-3차-고도화-mysql-공간-객체-활용)
  - [(4) 4차 고도화: read/write - through 패턴 구현, l1/l2 캐싱](#4-4차-고도화-readwrite---through-패턴-구현-l1l2-캐싱)

- [2️⃣ '매칭 User flow 시나리오 테스트 15회와 그 개선 과정'](#2️⃣-매칭-user-flow-시나리오-테스트-15회와-그-개선-과정)
  - [(0) 테스트 목표 설정](#0-테스트-목표-설정)
  - [(0) 테스트 001](#1-테스트-001)
  - [(1) 테스트 002: was, os 튜닝 후](#2-테스트-002-was-os-튜닝-후)
  - [(2) 테스트 003: 알림 전송 서비스에 retry 로직 추가 후](#3-테스트-003-알림-전송-서비스에-retry-로직-추가-후)
  - [(3) 테스트 004: 지연 변이 로직 구현 후](#4-테스트-004-지연-변이-로직-구현-후)
  - [(4) 테스트 005 ~ 010: 톰캣 thread 수와 db connection 의 연관 관계](#5-테스트-005--010-톰캣-thread-수와-db-connection-의-연관-관계)
  - [(5) 테스트 016: sql 진입점 로깅, 에러 수집, 슬로우 쿼리 확인](#6-테스트-016-sql-진입점-로깅-에러-수집-슬로우-쿼리-확인)
...
## (4) 4차 고도화: read/write - through 패턴 구현, L1/L2 캐싱
...
- `MAX RPS` = 5000,
- `RPS 증가 시점` = 매 30초마다 `RPS 300`씩 증가
